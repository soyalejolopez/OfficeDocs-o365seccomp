...
Currently, you can't disable mailbox auditing for specific mailboxes when mailbox auditing on by default is enabled for your organization. For example, setting the *AuditEnabled* mailbox property to **False** is ignored.  However, you can still use the **Set-MailboxAuditBypassAssociation** cmdlet to prevent mailbox actions performed by specific users from being logged. When you set a mailbox audit logging bypass, it prevents the actions of a specific user from being audited, regardless of which mailbox those actions are being performed on. If bypass mailbox auditing for a specific user, then the owner actions performed by that user won't be logged. And if that same user is assigned permissions to another users mailbox, then the delegate actions performed by the first user also won't be logged.

To bypass mailbox audit logging for a specific user, run the following command in Exchange Online PowerShell:
...
A value of **True** indicates that mailbox audit logging is bypass for that user.
